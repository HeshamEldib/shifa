...
function TooltipIcon({ label, glyph, color = "aqua" }) {
  const id = useMemo(() => `tip_${Math.random().toString(16).slice(2)}`, []);
  return (
    <button className={`pj-icon pj-${color}`} aria-describedby={id} type="button">
      <span className="pj-glyph" aria-hidden="true">
        {glyph}
      </span>
      <span className="sr-only">{label}</span>
      <span id={id} role="tooltip" className="pj-tooltip">
        {label}
      </span>
    </button>
  );
